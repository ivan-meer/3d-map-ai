...
4. Press Enter. The app will rebuild automatically.`;
  ...
  initializeMap() {
    ...
    if (sharedJourneyParam) {
      try {
        const decodedJson = decodeURIComponent(atob(sharedJourneyParam));
        const parsedBookmarks = JSON.parse(decodedJson);
        if (Array.isArray(parsedBookmarks) && parsedBookmarks.length > 0) {
          this.bookmarks = [...parsedBookmarks].reverse();
        ...
        console.error('Failed to parse shared journey:', e);
      }
    }
  }
  ...
  shareJourney() {
    ...
    try {
      ...
      const base64Encoded = btoa(encodeURIComponent(jsonStr));
      const shareUrl = new URL(window.location.href);
      shareUrl.searchParams.set('sharedJourney', base64Encoded);
      shareUrl.searchParams.delete('lat');
      shareUrl.searchParams.delete('lng');
      shareUrl.searchParams.delete('tilt');
      shareUrl.searchParams.delete('heading');
      shareUrl.searchParams.delete('range');
        ...
        <div class="timeline-header">
          ...
          <div style="display: flex; align-items: center; gap: 8px;">
            ...
            ${chronologicalBookmarks.length > 0 ? html`
              <button 
                class="timeline-filter-btn ${this.shareCopied ? 'active' : ''}"
                @click=${this.shareJourney}
                title="Generate a URL containing all encoded bookmark coordinates in this journey and copy it to clipboard"
                style="gap: 6px;">
                ${this.shareCopied ? html`
                  <svg xmlns="http://www.w3.org/2000/svg" height="14px" viewBox="0 -960 960 960" width="14px" fill="#10b981">
                    <path d="M382-240 154-468l57-57 171 171 367-367 57 57-424 424Z"/>
                  </svg>
                  <span style="color: #10b981; font-weight: bold;">Link Copied!</span>
                ` : html`
                  <svg xmlns="http://www.w3.org/2000/svg" height="14px" viewBox="0 -960 960 960" width="14px" fill="currentColor">
                    <path d="M720-80q-50 0-85-35t-35-85q0-7 1-14.5t3-13.5L322-392q-17 15-38 23.5t-44 8.5q-50 0-85-35t-35-85q0-50 35-85t85-35q23 0 44 8.5t38 23.5l282-179q-2-6-3-13.5t-1-14.5q0-50 35-85t85-35q50 0 85 35t35 85q0 50-35 85t-85 35q-23 0-44-8.5T638-568L356-389q2 6 3 13.5t1 14.5q0 7-1 14.5t-3 13.5l282 179q17-15 38-23.5t44-8.5q50 0 85 35t35 85q0 50-35 85t-85 35Zm0-640q17 0 28.5-11.5T760-760q0-17-11.5-28.5T720-800q-17 0-28.5 11.5T680-760q0 17 11.5 28.5T720-720ZM240-440q17 0 28.5-11.5T280-480q0-17-11.5-28.5T240-520q-17 0-28.5 11.5T200-480q0 17 11.5 28.5T240-440Zm480 280q17 0 28.5-11.5T760-200q0-17-11.5-28.5T720-240q-17 0-28.5 11.5T680-200q0 17 11.5 28.5T720-160Z"/>
                  </svg>
